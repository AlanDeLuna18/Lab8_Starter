...
function initializeServiceWorker() {
  ...
  if ("serviceWorker" in navigator)
  {
    window.addEventListener("load",async() => {
      try {
        const registeredWorker = await navigator.serviceWorker.register('/sw.js', {
            scope: "/",
          });
          ...
          else if (registeredWorker.waiting) 
          {
            console.log("Service worker installed");
          } 
          else if (registeredWorker.active) 
          {
            console.log("Service worker active");
          }
          else
          {
            console.log("Service Worker Unsuccessful"); 
          }
...
async function getRecipes() {
  ...
  //let finalRecipes = JSON.parse(recipesJSON); 
  ...
  /**************************/
    ...
    for(let a = 0; a < RECIPE_URLS.length; a++)
    {
      let currentRecipe = RECIPE_URLS[a]; 

      //A5
      try
      {
        //A6
        let URL = await fetch(currentRecipe); 

        //A7
        let currentJSON = await URL.json(); 

        //A8
        fetchedRecipes.push(currentJSON);

        //A9
        if(a == RECIPE_URLS.length - 1)
        {
